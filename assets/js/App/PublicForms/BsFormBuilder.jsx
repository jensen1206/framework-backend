...
import CreditCardType from "./components/Types/CreditCardType";
import {v4 as uuidv4} from "uuid";

const sleep = ms =>
    new Promise(resolve => setTimeout(resolve, ms));
export default class BsFormBuilder extends React.Component {
    constructor(props) {
        super(props);
        this.props = props;
        this.builderForm = React.createRef();
        this.builderParent = React.createRef();
        this.builderEmailSpam = React.createRef();
        this.builderNameSpam = React.createRef();
        this.builderTermsSpam = React.createRef();
        this.state = {
            formValidated: false,
            showErrorMsg: false,
            tinyMceRef: '',
    ...
    onSubmitBsForm(event) {
        ...
        sleep(50).then(() => {
            ...
            this.props.sendAxiosFormdata(formData, false)
        });
    }
    ...
    render() {
        ...
        return (
            <Fragment>
                                            ...
                                            return (
                                                <Fragment key={bIndex}>
                                                    {b.grid.map((g, gIndex) => {
                                                        return (
                                                            <div key={gIndex}
                                                                 className={"col-12" + (this.props.settings.col ? ' col-' + this.props.settings.col + '-' + g.col : '')}>
                                                                {g.forms.map((f, fIndex) => {
                                                                    return (
                                                                        <Fragment key={fIndex}>
                                                                            {this.props.onFindConditionForms(f.condition.type || '') ? (<>
                                                                                ...
                                                                                {f.type === 'credit-card' ||
                                                                                f.type === 'credit-card-date' ||
                                                                                f.type === 'credit-card-cvc'
                                                                                    ? (
                                                                                        <CreditCardType
                                                                                            form={f}
                                                                                            onSetFormCreditCardTypes={this.props.onSetFormCreditCardTypes}
                                                                                        />
                                                                                    ) : ''}
                                                                            </>) : (
                                                                                <div data-id={f.condition.type || ''}
                ...
                <div className=" load send-form-info mt-2">
                    <div className="sending-wait"></div>
                       <div className="send text-muted">Formular wird gesendet</div>
                </div>) : ''}
